...
const LeadGenNavbar = ({ onSearch }) => {
    ...
    const handleMarkAsRead = async (notification) => {
        try {
            const response = await fetch(`http://localhost:4000/api/notifications/${notification._id}/leadgen`, {
                method: 'PATCH',
                headers: {
            ...
            if (response.ok) {
                setNotifications(notifications.map(n => n._id === notification._id ? { ...n, readByLeadGen: true } : n));
    ...
    const filteredNotifications = filter === "all" ? notifications : notifications.filter(n => !n.readByLeadGen);
                ...
                >
                    <Box p={2} backgroundColor="#212e4a">
                        ...
                        {filteredNotifications.map((notification) => (
                            ...
                            >
                                <Paper sx={{ p: 2, width: '100%', backgroundColor: '#041926', }} elevation={notification.readByLeadGen ? 1 : 4}>
                                    <Box display="flex" alignItems="center" mb={2}>
                                        <Avatar sx={{ bgcolor: '#3e4396', mr: 2 }}>{notification.message.charAt(0)}</Avatar>
                                        <Box>
                                            <Typography variant="body1" sx={{ color: '#94e2cd', fontWeight: notification.readByLeadGen ? 'normal' : 'bold' }}>
                                    ...
                                    <Typography variant="caption" color="#e0e0e0" sx={{ display: 'block', textAlign: 'right' }}>
                                        {notification.readByLeadGen ? 'Read' : 'Unread'}
                                    </Typography>
                                </Paper>
                            </MenuItem>
                        ))}
            ...
            <Dialog open={openDialog} onClose={handleDialogClose} maxWidth="md" className="bounce-in-modal">
                ...
                <DialogContent>
                    {selectedNotification && (
                        <Paper sx={{ p: 3, width: '100%', backgroundColor: '#041926' }} elevation={selectedNotification.readByLeadGen ? 1 : 4}>
                            <Box display="flex" alignItems="center" mb={2}>
                                <Avatar sx={{ bgcolor: '#3e4396', mr: 3 }}>{selectedNotification.message.charAt(0)}</Avatar>
                                <Box>
                                    <Typography variant="h6" sx={{ color: '#94e2cd', fontWeight: selectedNotification.readByLeadGen ? 'normal' : 'bold' }}>
                            ...
                            <Typography variant="body2" color="#e0e0e0" sx={{ display: 'block', textAlign: 'right' }}>
                                {selectedNotification.readByLeadGen ? 'Read' : 'Unread'}
                            </Typography>
                        </Paper>
                    )}
                </DialogContent>
