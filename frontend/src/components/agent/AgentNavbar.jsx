...
const AgentNavbar = ({ onSearch }) => {
    ...
    const handleMarkAsRead = async (notification) => {
        try {
            const response = await fetch(`http://localhost:4000/api/notifications/${notification._id}/agent`, {
                method: 'PATCH',
                headers: {
            ...
            if (response.ok) {
                setNotifications(notifications.map(n => n._id === notification._id ? { ...n, readByAgent: true } : n));
    ...
    const filteredNotifications = filter === "all" ? notifications : notifications.filter(n => !n.readByAgent);
                ...
                >
                    <Box p={2} backgroundColor="#212e4a">
                        ...
                        {filteredNotifications.map((notification) => (
                            ...
                            >
                                <Paper sx={{ p: 2, width: '100%', backgroundColor: '#041926', }} elevation={notification.readByAgent ? 1 : 4}>
                                    <Box display="flex" alignItems="center" mb={2}>
                                        <Avatar sx={{ bgcolor: '#3e4396', mr: 2 }}>{notification.message.charAt(0)}</Avatar>
                                        <Box>
                                            <Typography variant="body1" sx={{ color: '#94e2cd', fontWeight: notification.readByAgent ? 'normal' : 'bold' }}>
                                    ...
                                    <Typography variant="caption" color="#e0e0e0" sx={{ display: 'block', textAlign: 'right' }}>
                                        {notification.readByAgent ? 'Read' : 'Unread'}
                                    </Typography>
                                </Paper>
                            </MenuItem>
                        ))}
            ...
            <Dialog open={openDialog} onClose={handleDialogClose} maxWidth="md" className="bounce-in-modal">
                ...
                <DialogContent>
                    {selectedNotification && (
                        <Paper sx={{ p: 3, width: '100%', backgroundColor: '#041926' }} elevation={selectedNotification.readByAgent ? 1 : 4}>
                            <Box display="flex" alignItems="center" mb={2}>
                                <Avatar sx={{ bgcolor: '#3e4396', mr: 3 }}>{selectedNotification.message.charAt(0)}</Avatar>
                                <Box>
                                    <Typography variant="h6" sx={{ color: '#94e2cd', fontWeight: selectedNotification.readByAgent ? 'normal' : 'bold' }}>
                            ...
                            <Typography variant="body2" color="#e0e0e0" sx={{ display: 'block', textAlign: 'right' }}>
                                {selectedNotification.readByAgent ? 'Read' : 'Unread'}
                            </Typography>
                        </Paper>
                    )}
                </DialogContent>
